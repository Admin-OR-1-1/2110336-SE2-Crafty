...
const FeedPreviewCard = ({ post }: FeedPreviewCardProps) => {
  const [isBanned, setIsBanned] = useState<boolean>(post.isBanned);

  const [banning, setBanning] = useState<boolean>(false);

  useEffect(() => {
    setIsBanned(post.isBanned);
  }, [post.isBanned]);

  const banPostById = async (id: string) => {
    setBanning(true);
    try {
      const response = await apiService.banPostById(id);
  ...
  const unbanPostById = async (id: string) => {
    setBanning(true);
    try {
      const response = await apiService.unbanPostById(id);
      if (response.status === ApiStatus.SUCCESS) {
        setIsBanned(false);
      }
    } catch (error) {
    ...
    <div className="flex h-fit w-full flex-row items-center gap-4 rounded-lg bg-white p-4 px-6">
      ...
      <div className="ml-auto flex flex-row items-center gap-3 max-md:flex-col">
        ...
        <button
          ...
          onClick={() => (isBanned ? unbanPostById(post.id) : banPostById(post.id))}>
          {isBanned ? 'Unban' : 'Ban'}
        </button>
